...
import { getProductBySlug, getProducts, addToCart, checkFavorite, toggleFavorite, getPromos, Product, Promo } from "@/lib/api";
...
export default function ProductDetailPage() {
  ...
  const [applicablePromos, setApplicablePromos] = useState<Promo[]>([]);
  ...
  async function loadProduct() {
    ...
    if (productRes.data) {
      ...
      // Fetch promos
      const promoRes = await getPromos();
      if (promoRes.data) {
        const productCategory = productRes.data.categoryId;
        const productId = productRes.data.id;
        const promos = promoRes.data.filter(p => 
          p.isActive && 
          new Date(p.endDate) > new Date() &&
          (
            p.categories?.some(c => c.id === productCategory) || 
            p.products?.some(prod => prod.id === productId) ||
                    ...
                    {applicablePromos.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {applicablePromos.map(promo => (
                          <div key={promo.id} className="inline-flex items-center gap-1.5 rounded-lg bg-sky-50 px-2.5 py-1 text-xs font-bold text-sky-700 border border-sky-100">
            ...
            <div className="lg:col-span-3">
              <div className="sticky top-20 space-y-3">
                <div className="rounded-lg border border-zinc-200 bg-white p-5">
                  ...
                  <div className="mt-4 space-y-2">
                    <Button
                      className="h-10 w-full gap-2 bg-zinc-900 text-white text-sm font-medium hover:bg-zinc-800"
                    ...
                    >
                      {buyingNow && <Loader2 className="h-4 w-4 animate-spin" />}
