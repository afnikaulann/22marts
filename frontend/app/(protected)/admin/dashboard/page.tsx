    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-700 hidden sm:block font-medium">Selamat datang, {user?.name?.split(" ")[0] || "Admin"}!</p>
          </div>
        </div>
        <select
          value={dateRange}
        ...
        {stats.map((stat) => (
          <div key={stat.label} className="flex items-center gap-3 rounded-lg border border-zinc-200 bg-white p-3">
            ...
            <div className="flex-1">
              <p className="text-xs text-zinc-700 font-medium">{stat.label}</p>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-3 sm:p-4 lg:col-span-2">
          <div className="mb-3 sm:mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="sm:text-right">
              ...
              <p className="text-xs text-zinc-700 font-medium">{rangedOrders.length} pesanan</p>
            </div>
          </div>
          <div className="h-52">
            {mounted && (
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                <AreaChart data={revenueByDay}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: any, name: any) => [name === "pendapatan" ? formatPrice(Number(value) || 0) : value, name === "pendapatan" ? "Pendapatan" : "Pesanan"]}
                  />
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-3 sm:p-4">
          <div className="mb-3 sm:mb-4">
            ...
            <p className="text-xs text-zinc-700 font-medium">Distribusi saat ini</p>
          </div>
          <div className="h-36">
      ...
      <div className="grid gap-3 lg:grid-cols-5">
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-3 sm:p-4 lg:col-span-2">
          <div className="mb-3 sm:mb-4">
            ...
            <p className="text-xs text-zinc-700 font-medium">7 hari terakhir</p>
          </div>
          <div className="h-40">
            {mounted && (
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
        ...
        <div className="rounded-lg border border-zinc-200 bg-white lg:col-span-3 overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-100 bg-zinc-50/80 px-4 py-3">
            ...
            <button onClick={() => window.location.href = "/admin/pesanan"} className="flex items-center text-xs font-semibold text-zinc-700 hover:text-zinc-900">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-100 bg-zinc-50/80">
                  <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-zinc-700">No. Pesanan</th>
                  <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-zinc-700 hidden sm:table-cell">Pelanggan</th>
                  <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-zinc-700">Total</th>
                  <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-zinc-700">Status</th>
                </tr>
              </thead>
                      ...
                      <tr key={order.id} className="hover:bg-zinc-50/50 cursor-pointer" onClick={() => window.location.href = `/admin/pesanan/${order.orderId}`}>
                        <td className="px-4 py-3">
                          <div>
                            ...
                            <p className="text-xs text-zinc-600 font-medium">{timeAgo(order.createdAt)}</p>
                          </div>
                        </td>
                        <td className="px-4 py-3 hidden sm:table-cell">
                          <p className="text-sm font-medium text-zinc-900">{order.user?.name || "—"}</p>
                          <p className="text-xs text-zinc-700 font-medium">{order.user?.email || "—"}</p>
                        </td>
      ...
      <div className="rounded-lg border border-zinc-200 bg-white">
        <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3">
          ...
          <button onClick={() => window.location.href = "/admin/produk"} className="flex items-center text-xs font-semibold text-zinc-700 hover:text-zinc-900">
            Lihat Semua <ChevronRight className="h-3 w-3" />
          </button>
        </div>
            ...
            topProducts.map((product, i) => (
              <div key={product.name} className="flex items-center gap-3 rounded-lg border border-zinc-100 p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-zinc-700 font-medium">{product.sold} terjual • {formatCompact(product.revenue)}</p>
                </div>
              </div>
            ))
          )}
