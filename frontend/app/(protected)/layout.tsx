...
const userLinks = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/promo", label: "Voucher Saya", icon: Tag },
    ],
  },
];
...
export default function ProtectedLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50">
      {/* Sidebar - Desktop */}
      <aside className="hidden w-56 shrink-0 flex-col border-r border-zinc-200 bg-white lg:flex sticky top-0 h-screen print:hidden">
        {/* Search */}
      ...
      {/* Sidebar - Mobile */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-56 transform bg-white transition-transform lg:hidden print:hidden ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex h-14 items-center justify-between border-b border-zinc-200 px-3">
          <Link href="/">
            <Image src="/logobaru.png" alt="22Mart" width={120} height={40} className="h-8 w-auto" />
          </Link>
      ...
      <div className="flex flex-1 flex-col">
        {/* Header */}
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-zinc-200 bg-white px-3 lg:px-4 print:hidden">
          <button
